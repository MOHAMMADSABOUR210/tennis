...
class MiniCourt():
  def __init__(self,frame):
    self.drawing_rectngle_width = 250
    self.drawing_rectngle_height = 500
    self.buffer = 50
  ...
  def set_canvas_background_box_position(self, frame):
    ...
    self.end_y = self.buffer + self.drawing_rectngle_height
    self.start_x = self.end_x - self.drawing_rectngle_width
    self.start_y = self.end_y - self.drawing_rectngle_height

  def draw_court(self,frame):
    for i in range(0, len(self.drawing_key_points),2):
        x = int(self.drawing_key_points[i])
        y = int(self.drawing_key_points[i+1])
        cv2.circle(frame, (x,y),5, (254,5,120),-1)
  ...
  def draw_points_on_mini_court(self,frames,postions, color=(0,235,225)):
